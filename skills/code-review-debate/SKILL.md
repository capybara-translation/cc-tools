...
`ID` は提起順に `R1`, `R2`, ... と付番し、ラウンドをまたいでも変えない（新規指摘は末尾に追加）。subagent へ渡す台帳、ラウンド間の参照、結論の出力すべてでこの ID を使う。
状態の遷移:
...
> - （再レビュー時）渡された指摘台帳の各係争項目について、台帳 ID（R1 など）を先頭に明示した上で `CONCEDE` か `HOLD` を判定し、`HOLD` には新たな根拠を添える。実装者の反論が妥当なら潔く `CONCEDE` せよ。新規指摘には ID を付けず `NEW` と明記する（付番は司会が行う）。

### 実装者への指示（subagent に渡すプロンプトの骨子）

> あなたは変更の実装者として、レビュー指摘の妥当性を検証する。ただし**反射的に防御してはならない。妥当な指摘は認めよ。**
> - まず `<差分取得コマンド>` を実行して差分を取得し、必要に応じて関連ファイルを読む。
> - 各指摘について、台帳 ID（R1 など）を先頭に明示した上で `ACCEPT`（妥当・要修正）/ `REJECT`（誤検知）/ `PARTIAL`（一部妥当）を判定する。ID を省略したり付け替えたりしない。
...
## 却下された指摘（誤検知・対象外）
- **[R7][元重要度] タイトル** (`file:行`): 却下理由（実装者の反論と、レビュワーが CONCEDE した根拠）
...
## ディスカッションログ（要約）
- ラウンド1: レビュワーが N 件提起（R1〜RN）→ 実装者が ACCEPT x / REJECT y / PARTIAL z（係争: R3, R7）
- ラウンド2: レビュワーが R7 を CONCEDE、R3 を HOLD → ...
...
- 結論・ログ・後続の会話で指摘を参照するときは必ず台帳 ID（R1 など）を使い、ID は上記フォーマットの見出しで必ずタイトルと対応付ける。ID だけで参照して対応表を省略しない。
